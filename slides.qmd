...
`luz` was designed to make deep learning with `torch` as effortless as possible, while at the same time allowing for easy customization. `luz` helps:

* Handling the usual three stages of machine learning
  * Training
  * Validation
  * Testing
* Maintaining flexibility with callbacks
* Managing data flow between *devices* (CPU and GPU, if you have one).
...
**How `luz` helps with devices**

For thecall to `predict()`, what happened "under the hood" was the following:
...
-   First training: We take the convnet from three chapters ago, and add dropout layers.

-   Second training: We replace dropout by batch normalization. (Everything else stays the same.)

-   Third training: We replace the model completely, by one chaining a pre-trained feature classifier (ResNet) and a small sequential model.
